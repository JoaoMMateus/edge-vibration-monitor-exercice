...
def generate_csv_stream(
    ...
    """
    Generate a stream from a CSV file.
    
    Yields (timestamp, value) tuples.
    ...
    with open(csv_path, 'r') as f:
        ...
        for row in reader:
            ...
            if not row[value_column].strip():
                continue
            
            try:
                timestamp = float(row[timestamp_column].strip())
                # Convert datetime string to timestamp if needed
                if ';' in str(timestamp) or '-' in str(timestamp):
                    from datetime import datetime
                    dt = datetime.strptime(row[timestamp_column].strip(), "%Y-%m-%d %H:%M:%S.%f")
...
class CSVSensor:
    """Wrapper to simulate sensor stream from CSV file."""
    ...
    def stream(self) -> Iterator[SensorSample]:
        """Generate samples from CSV as SensorSample objects."""
        for timestamp, value in generate_csv_stream(
            self.csv_path,
            value_column=self.value_column,
            timestamp_column=self.timestamp_column
        ):
            # For CSV data, we treat the value as acceleration
            # (simplified model - we only have one column)
            yield SensorSample(
                timestamp=timestamp,
                displacement=0.0,  # Not available in CSV
                velocity=0.0,     # Not available in CSV
                acceleration=value
            )
